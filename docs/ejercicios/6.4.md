...
Es importante para que tanto otros programadores como para nosotros en un futuro, puedan entender qué hace nuestro código sin tener que leer cada línea o perder tiempo adivinando para qué sirve cada 
variable y función. La diferencia es que los comentarios se escriben usando `//` y van dirigidos a la persona que lee el código. En cambio, la documentación se escribe con formato KDoc `/** ... */` 
y sirve como manual de instrucciones.

b) Describe **dos ejemplos** de comentarios importantes y necesarios (según la teoría) y enlaza a tu código.

RESPUESTA:

En el método normalizarNombre de la clase RegistroPersonas. He añadido un comentario que explica que se hace un `trim()` y se pone en minúscula porque "es una regla de negocio clave para no ensuciar 
BBDD y no guardar juan y Juan como dos distintos". Ese porque no es obvio al ver solo el código. En InformeCsv, hay un `.replace(",", ";")`. He añadido el por qué: "Reemplazamos la coma por punto 
y coma internamente para no romper el formato CSV". Si no pongo esto alguien podría borrar esa línea pensando que es un error ortográfico y se cargaría el fichero.

c) Describe **dos ejemplos** de comentarios/documentación no importante o innecesaria y enlaza a tu código (antes/después).

RESPUESTA:
He borrado 2 que estaban tal cual venía el código base:

1. `// Crea el StringBuilder` justo encima de un `val sb = StringBuilder()`.
2. `// devolver el string` junto a `return sb.toString()`.
...
Si quitásemos eso, Dokka la guardaría por defecto en escondida dentro de la carpeta `build/dokka/html`.

## Evidencias

- Captura o descripción de la documentación HTML generada en `Doc/`.
  RESPUESTA: Después de ejecutar el `./gradlew dokkaHtml`, he visto que dentro de la carpeta Doc se han creado hojas de estilo y varios ficheros `.html`.
...
Alumno Secundario nombre=Luis edad=19 curso=1DAM
